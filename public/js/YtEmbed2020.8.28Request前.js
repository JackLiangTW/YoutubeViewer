...
$(document).on("click",'#FixedVideoList .AddLinkPanel .bottoms button.add',async function(){
    ...
    //await YtApiForKeyWordSearch(Hrefs);//搜尋 Video
});
...
$(document).on('submit','#SearchPannel',async function(e){//搜尋影片 FormSubmit
    ...
    let Res=await YtApiForKeyWordSearch($('#SearchPannel input').val(),$('#SearchType').val());
    if(Res!==false){
        //console.log(Res);
        ClearAllContent();
...
function YtApiForKeyWordSearch(Wdd,Typp){//Yt Search Api (Not Finished)
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube PlayList 所有 item Video
...
function YtApiForGetComment(VIID,NB){//Api For Video Comment    
    if($(`#Outer .videoplayer:nth-child(${NB+1}) div.commentArea`).length>0){        
        $(`#Outer .videoplayer:nth-child(${NB+1}) div.commentArea`).toggleClass('hide');
        return false;
    }
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube APi 所有 Trending Video
