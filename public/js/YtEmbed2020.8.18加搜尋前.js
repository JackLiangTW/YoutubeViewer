    ...
    for(var s=NowEles;s<NowEles+LoadVideoNumbers&&s<VideoIdList.length;s++){
        ...
        <div key="${s}" class="videoplayer">
            ...
            <div>
                ...
                <p>觀看次數:${VideoInfoList[s]['views']}</p>
...
function YtApiForKeyWordSearch(Wdd){//2020.8.17 Yt Search Api (Not Finished)
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube PlayList 所有 item Video
            url: `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${Wdd}
                &maxResults=50&order=date&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI`,
            type:'GET',        
            dataType: "json",
            success: function(result){     
                console.log(result.items);                
