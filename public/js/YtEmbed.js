...
var WindowIsMobileSize=false;//是手機版本 size
var CanLoadMoreContentVideo=true;//可以scroll載入更多video
...
$(window).scroll(function(){
    ...
    if($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
        if(CanLoadMoreContentVideo&&IsYoutubePage)ScrollLoadMoreYouTubeIframe();//Scroll Load More Video
    }
    JudgeMenuEleInSight();//滾動超過 原本menu高度
    LoopContentVideoJudgeInSight();//在youtube page時 scroll auto play
});
function BuildYouTubeIframeAPIReady(ListArr,InfoArr) {//Init Build Content Video & Save ContentVideo All Data
    VideoIdList=ListArr;
    VideoInfoList=InfoArr;
    PlayerArray=[];
    PlayerArrayNotAutoPlay=[];
    IsAutoPlayScroll=false;
    for(var s=0;s<3&&s<ListArr.length;s++){
        let TimeElem='';
        let ViewsElem='';
        //if(InfoArr[s]['views']&&InfoArr[s]['views']!==false)ViewsElem=`<p>觀看次數:${InfoArr[s]['views']}</p>`;
        if(InfoArr[s]['views'])ViewsElem=`<p>觀看次數:${InfoArr[s]['views']}</p>`;
        if(InfoArr[s]['time'])TimeElem=`<p>${InfoArr[s]['time']}</p>`;
        $('#Outer').append(`
        ...
        `);
        /*
        <button class='fivideobtn' onclick='YtApiForGetComment("${ListArr[s]}",${s})'>
        留言區</button> */
    }
    for(var s=0;s<3&&s<ListArr.length;s++){
...
function ScrollLoadMoreYouTubeIframe() {//Content Video Area載入更多Video Ele
    //console.log('Trigger More Append');
    CanLoadMoreContentVideo=false;
    let NowEles=PlayerArray.length;
    let LoadVideoNumbers=3;//一次加載影片數 PC
    if(mobileCheck())LoadVideoNumbers=2;//一次加載影片數 Mobile
    for(var s=NowEles;s<NowEles+LoadVideoNumbers&&s<VideoIdList.length;s++){
        let ViewsElem='';
        if(VideoInfoList[s]['views'])ViewsElem=`<p>觀看次數:${VideoInfoList[s]['views']}</p>`;
        $('#Outer').append(`
        <div key="${s}" class="videoplayer">
            <div id="player${s}"></div>
            <div>
                <p>${VideoInfoList[s]['title']}</p>
                <p>${VideoInfoList[s]['time']}</p>
                ${ViewsElem}
                <p>
                    <button class='fivideobtn' onclick='FixedPlayerBuildYoutubeVideo("${VideoIdList[s]}","channel")'>
                    使用撥放器</button>
                    <button class='fivideobtn' onclick='FixedPlayerBuildYoutubeVideo("${VideoIdList[s]}","personal")'>
                    加入自訂歌單</button>
                </p>
                </p>
            </div>
        ...
        `);
        /*
        <button class='fivideobtn' onclick='YtApiForGetComment("${VideoIdList[s]}",${s})'>
        留言區</button>*/
    }
    for(var s=NowEles;s<NowEles+LoadVideoNumbers&&s<VideoIdList.length;s++){
        let Wdd=parseInt($('.videoplayer').width());
        ...
        var LitPlayer = new YT.Player(`player${s}`, {
            height: `${Wdd*0.6}`,
            width: `${Wdd}`,
            videoId: `${VideoIdList[s]}`,
            playerVars: { //自訂參數
                'loop':1,
                'playlist':`${VideoIdList[s]}`,
                'rel': 0,//不顯示相關視頻
                'playsinline': 1,//在iOS的播放器中全屏播放，0:全屏(默認)，1:內嵌
                },
            events: {
...
function onPlayerReady(event) {
    ...
    if(PlayerArray[0].getPlayerState()!==1&&IsFixedVideoPlaying==false){//event.target.getPlaylist()[0]==PlayerArray[0].getPlaylist()[0]&&
        if(iOS()==true||isWebview()){//---For Apple Device AutoPlay
            PlayerArray[0].mute();
        }
...
function PausePlayingSingleVideo(){//暫停 正在撥放的video
    //console.log(YtNowVideo);
    //PlayerArray[YtNowVideo].pauseVideo();
    if(PlayerArray[YtNowVideo])PlayerArray[YtNowVideo].pauseVideo();
}
function PlaySingleVideo(N){//停止其他video 播單支
    ...
    &&PlayerArray[N].getPlayerState()!==1){//如果第N不再禁止AutoPlay名單 && 第N 沒有正在撥放
        if(iOS()==true||isWebview())PlayerArray[N].mute();//---For Apple Device AutoPlay
...
function FixedPlayerBuildYoutubeVideo(ViID,KIND){//建立 Yt Fixed PlayerElement
    ...
    if(FixedYtElem==null){//Fixed VideoPlayer Not Exisit
        ...
        if(VideoIdList.indexOf(ViID)!=-1&&PlayerArray.length>=VideoIdList.indexOf(ViID)&&KIND!=='ShareChannel'){
            //帶時間給 Fixed撥放器
            StartVideoSeconds=parseInt(PlayerArray[VideoIdList.indexOf(ViID)].getCurrentTime());
        }
...
function FixedonPlayerReady(event){//Fixed Video 初始化play
    ...
    if(iOS()==true||isWebview())event.target.mute();//---For Apple Device AutoPlay
    event.target.playVideo();
...
function InitRenderFixedVideoList(){//初始化Render Fixed List
    ...
    <div id='FixedVideoList' class='${FixedListClass}'>
        ...
        <div class='AddLinkPanel'>
            <div class='tops'>
                <button class='show'>撥放Youtube清單</button>
                <button>加入影片</button>
            </div>
            <div class='bottoms'>
                <div class='show'>
                    <h1>撥放Youtube PlayList/清單</h1>
                    <input type="text" class="Lists" placeholder="輸入清單網址">
                    <button class='add'>確定</button>
                </div>
                <div>
                    <h1>自訂歌單加入Youtube影片</h1>
                    <input type="text" class="links" placeholder="請輸入影片網址">
                    <input type="text" class="title" placeholder="請輸入此影片標題">
                    <button class='add'>確定</button>
                </div>
            </div>
        </div>
    ...
    <div class='AddLinkPanel'>
            <div class='bottoms'>
                <div class='show'>
                    ...
                    <input type="text" class="links" placeholder="請輸入Youtube影片">
...
$(document).on("click",'#FixedVideoList .AddLinkPanel .bottoms button.add',async function(){
    if($('.AddLinkPanel>.tops>button').eq(0).hasClass('show')){
        let Hrefs=$('.AddLinkPanel .bottoms .Lists').val();
        await ParseAndApiGetYtListData(Hrefs);//Api Get Yt PlayList all Video
    }else{
        let Hrefs=$('.AddLinkPanel .bottoms .links').val();
        let Titles=$('.AddLinkPanel .bottoms .title').val();
        JudgeAndInsertOneVideoPersonlList(Hrefs,Titles);//輸入自訂YtVideoUrl
    }
});
function JudgeAndInsertOneVideoPersonlList(Hrefs,Titles){//自訂List(判斷&新增)單支Video
    ...
    if(Hrefs.includes('/youtu.be/')){//短網址
        var slicenb1=Hrefs.indexOf('/youtu.be/')+10;
        Hrefs=Hrefs.slice(slicenb1,slicenb1+11);
    }else if(Hrefs.includes('youtube.com/watch?v=')){
...
function getParameterByName(name, url) {//Get Url Query Value
...
//------------Do Yt Api Get Data----------------------------
$(document).on('submit','#SearchPannel',async function(e){//搜尋影片 FormSubmit
    e.preventDefault();
    ...
    DoApiRequestYtPage();
    // if($('#SearchType').val()!=='channel'){//關鍵字找 video ----用API
    //     let Res=await YtApiForKeyWordSearchVideo($('#SearchPannel input').val(),$('#SearchType').val());
    //     if(Res!==false){
    //         ClearAllContent();
    //         //$('#SocialKind>button').eq(0).addClass('ISactive').siblings('button').removeClass('ISactive');
    //         $('#SocialKind>button').removeClass('ISactive');
    //         $('#PostClassSelect').css('display','none');
    //         $('#SelectShow').css('display','none');
    //         ClearVarAndRenderApiData(Res);
    //     }
    // }else{//關鍵字找 channel (Api流量限制-停用 [1User搜尋1次=100q(10000q/day)])
    //     let Res=await YtApiForKeyWordSearchChannel($('#SearchPannel input').val());
    //     if(Res!==false){
    //         ClearAllContent();
    //         $('#SocialKind>button').removeClass('ISactive');
    //         $('#PostClassSelect').css('display','none');
    //         $('#SelectShow').css('display','none');
    //         RenderChannelDataList(Res);
    //     }
    // }
});
$('#SearchPannel').focusout(function() {//離開 focus搜尋輸入框 -> 關掉聯想
    if($('#SuggestList p.active').length==0)ClearSuggestUI();//清除 文字聯想UI
});
$(document).on('mouseenter','#SuggestList p',function(){
    $(this).addClass('active').siblings('p').removeClass('active');
});
$(document).on('mouseleave','#SuggestList p',function(){
    $(this).removeClass('active');
});
function DoApiRequestYtPage(KW=null){//Ajax for Request Video/Channel
    ClearSuggestUI();//清除 文字聯想UI
    if(KW!==null){//點擊文字聯想
        $('#SearchPannel input').val(`${KW}`);
    }
    let AjUrl="/YoutubeSearchChannelRequest";
    let IsShowVideo=false;
    if($('#SearchType').val()!=='channel'){
        AjUrl="/YoutubeSearchVideoRequest";
        IsShowVideo=true;
    }
    let ApiData={
        word:$('#SearchPannel input').val(),
        kind:$('#SearchType').val()
    }
    $.ajax({//[Request Ajax]初始化 抓今日data
        ...
        success: function(result){
            ...
            $('#SelectShow').css('display','none');
            $('#Outer').empty();
            if(result.data.word!==false){//建議關鍵字
                console.log(result.data.word);
                let ArrD=JSON.parse(result.data.word);
                let StrD='';
                ArrD.forEach((i)=>{
                    StrD=StrD+`${i['text']}`
                })
                // $('#Outer').append(`<p>此結果以:
                // <button onclick="DoApiRequestYtPage('${AjUrl}',${IsShowVideo},{'word':'${StrD}','kind':'${ApiData['kind']}'})">
                // ${StrD}</button></p>`);
                $('#Outer').append(`<p>此結果是以"${StrD}"為關鍵字搜尋</p>`);
            }
            if(IsShowVideo)ClearVarAndRenderApiData_Request(result.data);
            else if(IsShowVideo==false)RenderChannelDataList_Request(result.data);
        }}
    );
}
function ParseAndApiGetYtListData(TheUrl){//Api Get Yt PlayList 中所有 Video(最多50筆)
    return new Promise(async function(resolve,reject){
        //let ListID=getParameterByName('list',TheUrl);
        let ListID=getParameterByName('list',TheUrl);
        if(ListID==null||ListID==''){
            alert('找不到PlayListId!!');
            return false;
        }
        $.ajax({//抓Youtube PlayList 所有 item Video
            url: `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${ListID}&maxResults=50&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI`,
            type:'GET',
            dataType: "json",
            success: function(result){
                //console.log(result.items);
                let NewFixedVideoChannelList=[];
                FixedVideoIdList=[...result.items].map((i)=>{
                    let OBJ={
                        videoId:i['snippet']['resourceId']['videoId'],
                        title:i['snippet']['title']
                    }
                    NewFixedVideoChannelList.push(OBJ);
                    return i['snippet']['resourceId']['videoId'];
                });
                FixedVideoChannelList=NewFixedVideoChannelList;
                FixedPlayerBuildYoutubeVideo(FixedVideoIdList[0],'ShareChannel');
                resolve();
            },
            error: function (e) {
                console.log(e);
...
function YtApiForKeyWordSearchVideo(Wdd,Typp){//Yt KeyWord Search Api For Videos
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube PlayList 所有 item Video
            url: `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${Wdd}
            &order=${Typp}&type=video&maxResults=50&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI`,
            ...
            error: function (e) {
                console.log(e);
                if(e.status=='403')alert('搜尋流量到達Youtube限制值!');
                else{alert('Search載入錯誤!');}
                resolve(false);
            }
        });
    });
}
function YtApiForKeyWordSearchChannel(Wdd){//Yt KeyWord Search Api For Channel
    return new Promise(async function(resolve,reject){
        console.log(Wdd);
        $.ajax({//抓Youtube PlayList 所有 item Video
            url: `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${Wdd}
            &order=relevance&type=channel&maxResults=50&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI`,
            type:'GET',
            dataType: "json",
            success: function(result){
                if(result.items.length==0){
...
function YtApiForGetVideosByChannelId(Idd){//取得 Channel Videos & Render them
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube PlayList 所有 item Video
            url: `https://www.googleapis.com/youtube/v3/search?channelId=${Idd}&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI&part=snippet,id&order=date&maxResults=50`,
            type:'GET',
            dataType: "json",
            success: function(result){
                ClearVarAndRenderApiData(result.items);
                // $('#Outer').append(`
                // <div class='ChnDetail'>
                // <p><img src="${Img}" width="60px" height="60px" alt="">
                // <span>${Intro}</span>
                // <button>收藏</button></div></p>`);
                // $('#Outer .ChnDetail').prependTo("#Outer");
                resolve();
            },
            error: function (e) {
                console.log(e);
...
function YtGetChannelVideosByUrl(URL){//[Request Ajax]取得 Channel的 Videos (用網址)
    $.ajax({//[Request Ajax]初始化 抓今日data
        url: '/GetYtChannelVideosRequest',
        type:'POST',
        contentType:"application/json",
        dataType:"json",
        ...
        success: function(result){
            console.log(result);
            if(result.data!==false){
                ClearVarAndRenderApiData_Request(result.data);
            }
        }}
    );
}
function YtApiForTrendingData(Region){//不同地區發燒影片 API
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube APi 所有 Trending Video
            url: `https://www.googleapis.com/youtube/v3/videos?part=snippet,id&chart=mostPopular&regionCode=${Region}&maxResults=50&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI`,
            type:'GET',
            dataType: "json",
            success: function(result){
                ClearVarAndRenderApiData(result.items);
...
function YtApiForGetComment(VIID,NB){//Api For Video Comment
    if($(`#Outer .videoplayer`).eq(NB).find(`div.commentArea`).length>0){
        $(`#Outer .videoplayer`).eq(NB).find(`div.commentArea`).toggleClass('hide');
        return false;
    }
    return new Promise(async function(resolve,reject){
        $.ajax({//抓Youtube APi 所有 Trending Video
            url: `https://www.googleapis.com/youtube/v3/commentThreads?videoId=${VIID}&key=AIzaSyA2E0JQK7A7o_wPDDkRYagTz6XdE7xuVzI&textFormat=plainText&part=snippet&maxResults=30&order=relevance`,
            type:'GET',
            dataType: "json",
            success: function(result){
                //console.log(result);
                resolve(result);
                let MsgItems=``;
                result.items.forEach((i,nb)=>{
                    let tt=`<p>Likes:
                    <span>${i['snippet']['topLevelComment']['snippet']['likeCount']}</span>
                    <span>${i['snippet']['topLevelComment']['snippet']['authorDisplayName']}</span>
                    <span>${i['snippet']['topLevelComment']['snippet']['textDisplay']}</span>
                    </p>`;
                    MsgItems=MsgItems+tt;
                });
                $('#Outer .videoplayer').eq(NB).append(`
                <div class='commentArea'>${MsgItems}</div>
                `)
            },
            error: function (e) {
                console.log(e);
                if(e.status='403')alert('該影片沒有留言功能或是Youtube流量到達上限!');
                else{alert('Search載入錯誤!');}
                resolve(false);
            }
        });
...
function ClearVarAndRenderApiData(Res){//清除 影片client data以及 Render Data Video
    $('#Outer').empty();
    IsYoutubePage=true;
    let OutArrID=[];
    let OutArrInfo=[...Res].map((i)=>{
        let Idd=i['id'];
        if(i['id']['videoId'])Idd=i['id']['videoId'];
        OutArrID.push(Idd);
        return{
            title:i['snippet']['title'],
            views:false,
    ...
    $("html, body").animate({ scrollTop: 0 },400);//Move To Top
    BuildYouTubeIframeAPIReady(OutArrID,OutArrInfo);
}
function ClearVarAndRenderApiData_Request(Res){//[RequestShow]清除var & Render Data Video (Request 版本)
    // $('#Outer').empty();
    IsYoutubePage=true;
    let OutArrID=[];
    let OutArrInfo=[...Res.data].map((i)=>{
        OutArrID.push(i['id']);
        return{
            title:i['name'],
            views:false,
...
function RenderChannelDataList(Res){//Render Channel List Data Button
    $('#ClassBtns').empty();
    $('#ClassBtns').css('display','block');
    let Ttx='';
    Res.forEach((i,nb)=>{
        if(nb==0){
            YtApiForGetVideosByChannelId(i['id']['channelId']);//取得第一個channel Videos
            Ttx=Ttx+`<button class="ISactive"
            onclick="YtApiForGetVideosByChannelId('${i['id']['channelId']}')">
            ${i['snippet']['channelTitle']}</button>`
        }else{
            //onclick="YtApiForGetVideosByChannelId('${i['id']['channelId']}','${i['snippet']['description']}','${i['snippet']['thumbnails']['default']['url']}')">
            Ttx=Ttx+`<button
            onclick="YtApiForGetVideosByChannelId('${i['id']['channelId']}')">
            ${i['snippet']['channelTitle']}</button>`
        }
    })
    $('#ClassBtns').append(`${Ttx}`);
    $("html, body").animate({ scrollTop: 0 },400);//Move To Top
    //BuildYouTubeIframeAPIReady(OutArrID,OutArrInfo);%
}
function RenderChannelDataList_Request(Res){//[RequestShow]Render Channel List Data Button
    $('#ClassBtns').empty();
    $('#ClassBtns').css('display','block');
    let Ttx='';
    Res.forEach((i,nb)=>{
        if(nb==0){
            YtGetChannelVideosByUrl(i['url']);//取得第一個channel Videos
            Ttx=Ttx+`<button class="ISactive"
            onclick="YtGetChannelVideosByUrl('${i['url']}')">
            ${i['name']}</button>`
        }else{
            Ttx=Ttx+`<button
            onclick="YtGetChannelVideosByUrl('${i['url']}')">
